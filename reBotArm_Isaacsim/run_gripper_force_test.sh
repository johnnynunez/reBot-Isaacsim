#!/usr/bin/env bash
# 夹爪受力测试启动脚本 / Gripper-force-test launcher.
# 使用 Isaac 官方 python.sh 运行 isaacsim_gripper_force_test.py。
# Run isaacsim_gripper_force_test.py via the official Isaac Sim python.sh.
set -eo pipefail
...
exec bash "${ISAACSIM_PYTHON}" "${SCRIPT_DIR}/isaacsim_gripper_force_test.py" "$@"
