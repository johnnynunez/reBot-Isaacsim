...
A("  Inertia tensors are unchanged from the URDF (PR#3 did not update them to match the new masses).")
...
A("- newton:velocityLimit (deg/s) authored by converter 0.3.0 replaces the old custom `urdf:limit:velocity`.")
A("- No MDL materials in 0.3.0 output (UsdPreviewSurface only); no legacy `payloads/` transformer package.")
A("- Post-export edits re-applied by `scripts/prep_asset.py`: drives (July gains), gripper convexDecomposition,")
A("  `newton:selfCollisionEnabled=0`, solver caps nconmax=8192/njmax=32768, Isaac robot schema.")
A("")
A("Evidence: `evidence/gt_pj_new_newton.json`, `evidence/gt_pj_new_physx.json`, `evidence/gravity_droop.json`,")
A("logs alongside. Harness: `scripts/gaintuner_perjoint_361.py` (+ `scripts/run_full_matrix.sh`).")
